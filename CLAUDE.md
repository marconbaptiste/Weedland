...
**Auto-inscription client (QR public)** : chaque magasin affiche au comptoir un **QR d'inscription** (`urlInscription(magasinId)` → route publique `/rejoindre/:magasinId`, page `RejoindreCarte.jsx`). Le client le scanne sur **son** téléphone, choisit un surnom + saisit **son téléphone** (consentement explicite via case à cocher), valide, et obtient aussitôt sa **carte de fidélité** publique (`/carte/:id`) à ajouter à l'écran d'accueil — avec **+1 étoile offerte** à l'inscription. Côté base, la fonction `inscription_client_publique(p_magasin, p_surnom, p_telephone)` (SECURITY DEFINER, ouverte en `anon`) crée la fiche dans le bon magasin, **déduplique par (magasin, téléphone)** (re-scanner ne crée pas de doublon ni de nouvelle étoile bonus) et n'expose jamais le téléphone publiquement. Le QR d'inscription s'affiche/imprime depuis la page **Clients** (bouton « QR d'inscription », `ModaleQRInscription.jsx`). Migration : `supabase/migrations/2026-06-23-inscription-client-publique.sql`.
